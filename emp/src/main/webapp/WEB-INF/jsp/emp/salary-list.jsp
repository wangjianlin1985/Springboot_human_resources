...
<section class="Hui-article-box">
	...
	<div class="Hui-article">
		<article class="cl pd-20 container" style="padding-top: 0; width: 90%;">
			<p>&nbsp;</p>
			<div style="width: 210px; float: left; margin: 8px 0;">
				<form action="salarymgr" method="get">
				<input type="text" class="input-text" placeholder="工资月份" name="yearMonth" onfocus="WdatePicker({dateFmt:'yyyy-MM',readOnly:true})" style="width: 120px;">
				<button class="btn btn-primary size-S" type="submit"><i class="Hui-iconfont" style="color: #fff;">&#xe665;</i>&nbsp;查询</button>
				</form>
			</div>
			<p class="c-warning" style="float: right; margin: 12px 0;">单位：元</p>
			<div class="mt-10">
				<table id="example" class="table table-bordered table-hover">
					<tbody>
						...
						<c:forEach items="${page.records }" var="row" varStatus="vs">
							<tr>
								<td>${row.salary.salaryDate }</td>
								<%-- <td><fmt:formatDate value="${r.publishTime }" pattern="yyyy-MM-dd HH:mm"/></td> --%>
								<td>${row.salary.baseAmt }</td>
								<td>${row.salary.mealAmt }</td>
								<td>${row.salary.houseAmt }</td>
								<%-- <td><p class="longtxt" style="width: 200px;" title="${r.title }">${r.title }</p></td> --%>
								<td>${row.salary.fullAtt }</td>
								<td>${row.salary.subsidy }</td>
								<td class="c-warning">-${row.salary.tax }</td>
								<td class="c-warning">-${row.salary.fine }</td>
								<td><strong class="c-success">${row.salary.actAmount }</strong></td>
								<td>
									<a href="/emp/emp/salarydetail?sid=${row.salary.salaryId }"><button class="btn btn-primary size-S"><i class="Hui-iconfont" style="color: #fff;">&#xe60c;</i>&nbsp;详情</button></a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
